"""
This code is the same as in HESE_fit.py but callable from another script, 
HESE_compiler.py
"""
"""
...
def HESE_fit_comp(args):
    ...
    for param, BF_param in zip(parameter_names, BF_params):
        ...
        if param == "astro_gamma": 
            param_astro_gama = BF_param
        elif param == "astro_norm":
            param_astro_norm = BF_param
            
    
    param_astro_gama_str = str(param_astro_gama)
    param_astro_norm_str = str(param_astro_norm)
    BF_llh_str           = str(BF_llh.real)
    file = open("fit_data.txt", "a")
    file.write("\n" + param_astro_gama_str + " " + param_astro_norm_str + " " + BF_llh_str)
    file.close()
